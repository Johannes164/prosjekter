...
function checkInput() {                                         //funksjon som viser feilmelding popup viss input er ugyldig
    if (isNaN(diskInput.value) || diskInput.value < 0 || diskInput.value > 9) { //sjekker om input er et tall mellom 0 og 9
...
function resetDisks() {                                         //funksjon som resetter tårnet til opprinnelig tilstand og sjekker om input er gyldig (brukes bl.a til reset knapp)
    ...
    solveButton.addEventListener("click", solveButtonSolve); //legger til eventlisteneren for solve knappen i tilfelle den er fjernet av solve knappen (kode rett under)
    solveButton.innerHTML = "Solve"; //endrer teksten på knappen tilbake til "Solve"
}

function solveButtonSolve() {                                //funksjon som løser tårnet når brukeren trykker på "solve" knappen (brukes til solve knapp)
    resetDisks();
    solveH(rods[0].dataset.disks, rods[0], rods[2], rods[1]);
    //fjern funksjonen for å legge den til igjen når den er ferdig
    solveButton.removeEventListener("click", solveButtonSolve); //fjerner eventlisteneren for å hindre at brukeren trykker på knappen flere ganger, den legges til igjen når solveH er ferdig
...
async function solveH(n, from, to, extra) {                     //rekursiv funksjon som løser hanoi-tårnet for n skiver, se prototype (i arbeidsprosess mappen) for mer info, eneste
    ...
    if (n === 1) {
        ...
        if (rods[2].dataset.disks === n) { //viss tredje stolpe har n skiver, er tårnet løst, så vi gir solveButton tilbake eventlistener
            solveButton.addEventListener("click", solveButtonSolve);
...
function selectrod(rod) {                                       //funksjon som lar brukeren velge en stolpe som skal flyttes fra og en stolpe som skal flyttes til (selectedRod er den som allerede er valgt, altså forrige valg, rod er den du tryker)
    ...
    if (selectedRod === rod) { //viss du trykker på samme stolpe
        ...
        if (rod.lastChild === null || parseInt(selectedDisk.id.slice(4)) > parseInt(rod.lastChild.id.slice(4))) {  //sjekker om trekket er lovlig, viss det er lovlig flyttes skiven og stolpene som kan klikkes får clickable
...
solveButton     .addEventListener("click", solveButtonSolve );  //legger til eventlistener på solve knappen             som kjører solveButtonSolve når den blir trykket på
